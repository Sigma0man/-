...
function kosoi_krest!(robot::CordBorderRobot)
    
    come_back!(robot)
    snake!(()->false,robot;start_side=Ost,ortogonal_side=Nord)
    snake!(()->isitstart(robot), robot; start_side=West, ortogonal_side=Sud)
    
end

rob = Robot("14.sit", animate = true)
r=CordBorderRobot(rob,0,0)
